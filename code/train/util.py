...
def z_normalize(data):
    new_data = data.copy()
    if len(data.shape) > 2:
        new_data = new_data.reshape(data.shape[0], data.shape[1] ** 2)
    ...
    print n
    avg = np.zeros(rows)
    normal_data = np.zeros_like(new_data, dtype=np.float64)
    for i in range(rows):
        print new_data[i, :]
        avg[i] = np.average(new_data[i, :])
        std[i] = np.std(new_data[i, :])
        print str(i) + "-> avg: " + str(avg[i]) + " std: " + str(std[i])
        for b in range(n):
            normal_data[i, b] = (new_data[i, b] - avg[i]) / float(std[i])
...
def get_labeled_data(pickle_name, train=True, reduced_dataset=False, classes=range(10), examples_per_class=100,
    ...
    if reduced_dataset:
        pickle_name = '_'.join(
            [pickle_name, 'reduced', '_'.join([str(class_) for class_ in classes]), str(examples_per_class),
             str(normalize_input)])

    if os.path.isfile('%s.pickle' % pickle_name) and not normalize_input:
...
def save_theta(weights_dir, populations, neuron_groups, ending, suffix):
    ...
    for population in populations:
        ...
        np.save(os.path.join(weights_dir, 'theta_' + population + '_' + ending + '_' + str(suffix)),
                neuron_groups[population + 'e'].theta)
